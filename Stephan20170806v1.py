...
for row, vs_Ticker in enumerate(df_info.loc[:10, 'Ticker']): # reads over the column called 'Ticker' 
#    print row, ' ', vs_Ticker
...
#        ps_Date = pd.to_datetime(df_Security['price_date'].values).date
        ps_Date = df_Security['price_date'].values
...
#        na_Dates_Testing_Set = dts.date2num(ps_Date[int(len(nai_Prices)/2):])
        ...
        # create dataframe the lists all prices and dates and stuff for TRAINING and TESTING
        df_Sec_Train = m_Dfm.f_CutDates(df_Security, ps_Date_Training_Set[0], ps_Date_Training_Set[-1])
        df_Sec_Test = m_Dfm.f_CutDates(df_Security, ps_Date_Testing_Set[0], ps_Date_Testing_Set[-1])
        ...
        BackTest = ClassBacktesting(df_Sec_Train, nai_Buy, ps_Date_Training_Set)  
        df_Returns, df_NrOfShares, df_ValueShares = BackTest.get_returns(vf_CASH)
        
        df_Track_Trades = pd.DataFrame(data = {'Buy_Sell_Signal': nai_Buy , 
                                               'Price' : nai_Prices_Training_Set,
                                               'price_date' : df_Returns['price_date'],
                                               'returns' : df_Returns['returns'],
                                               'value_shares': df_Returns['value_shares'], 
                                               'valuecash' : df_Returns['valuecash']})
...
#        dic_Results[vs_Ticker]= [df_Returns,
#                                  df_NrOfShares, 
#                                  df_ValueShares]
#        
#        if row == 0:
...
#                                  df_ValueShares]]
#        else:
#            l_Results.append([vs_Ticker, 
#                                  df_Returns,
#                                  df_NrOfShares, 
#                                  df_ValueShares])
                
#bla = df_Results.loc[3,'df_Returns']
        
#        l_Data = df_Security
#        vs_PriceType = 'adj_close_price'
#        
...
#        if len(l_Data) > 1:
#            df_Data = m_Dfm.merge_dfs([df.loc[:, ('price_date', vs_PriceType)] for df in l_Data], 'price_date', [vs_PriceType])
#        else:
#            df_Data = l_Data[0][['price_date', vs_PriceType]]
#        
#        na_Time = df_Data['price_date'].values
#        for vi_DataInd, dt_Date in enumerate(na_Time[0:10]):
#            print 'data ind:', vi_DataInd, ' dt_date = ', dt_Date, type(dt_Date)
#    
